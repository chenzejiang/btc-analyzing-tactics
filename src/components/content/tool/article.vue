<template>
    <div class="tool_box w1000">
        <div class="box404" v-show="code == 404">
            <h1 class="h1404" >404获取不到文章信息</h1>
        </div>
        <div class="rich_media_area_primary_inner" v-show="code == 200">
            <div id="img-content">
                <h2 class="rich_media_title" id="activity-name">{{title}}
                </h2>
                <div id="meta_content" class="rich_media_meta_list">
                    <span class="rich_media_meta rich_media_meta_text">王团长</span>
                    <span class="rich_media_meta rich_media_meta_nickname" id="profileBt">
                        <router-link id="js_name" :to="{path:'/tool/list'}">王团长区块链日记</router-link>
                    </span>
                    <em id="publish_time" class="rich_media_meta rich_media_meta_text">{{time}}</em>
                </div>
                <div class="rich_media_content articleBox" id="js_content"></div>
            </div>
        </div>
	</div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            onGetData(_id){
                ...
                $.ajax({
                    type: "get",
                    url: 'http://wtz.bishuju.top/api/article.php',
                    data: {
                        type: _id
                    },
                    dataType:"json",
                    success: function (data) {
                        ...
                        if(data["code"] == 404){
                            ...
                            $("#js_content").html(data["text"]);
                            that.time = data["time"];
                            that.title = data["title"];
                            $("#js_content img").each(function(i){
                                console.log($(this));
        ...
        mounted: function () {
            that = this;
            let url_id = that.$route.query.id;
            this.onGetData(url_id);
        }
    }
</script>
...
<style lang="less" rel="stylesheet/less" scoped>
    ...
    .video-package .video-close-button i,.video-package .video-provider-button i {
        position: relative;
        top: 1px
    }
